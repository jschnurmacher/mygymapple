...
namespace MyGym
{
    ...
    public partial class AccountScheduleChild : ContentPage
    {
        ...
        {
            GymMobile gym = (GymMobile)Application.Current.Properties["gym"];
            string s = string.Format(new CultureInfo(gym.Culture), "Upgrade your enrollment to unlimited for {0:c} per billing cycle. (you will be charged an initial prorated fee for the remaining days left in your billing cycle)", gym.UnlimitedFee);
        ...
        {
            var res = await DisplayAlert("Remove Unlimited Enrollment", "Remove the unlimited option from this enrollment", "OK", "Cancel");
            if (res == true)
            {
                int enrollId = Convert.ToInt32(((TappedEventArgs)e).Parameter);
                Xamarin.Essentials.Preferences.Set("enrollid", enrollId);
